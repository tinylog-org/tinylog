...
class ClassOnlyLocationInformationTest {
	...
	private static Jul_____Benchmark benchmark;
	...
	static void init() throws IOException {
		benchmark = new Jul_____Benchmark(LocationInfo.CLASS_OR_CATEGORY_ONLY);
		benchmark.configure();
	}
	...
	void output() throws InterruptedException {
		...
		assertThat(path)
			...
			.matches("\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2} - main - org\\.tinylog\\.benchmarks\\.logging\\."
				+ "jul_____\\.Jul_____Benchmark - INFO: Hello 42!" + NEW_LINE);
	}
