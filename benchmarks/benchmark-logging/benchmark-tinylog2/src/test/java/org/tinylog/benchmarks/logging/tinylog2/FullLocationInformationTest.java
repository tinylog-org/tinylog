...
class FullLocationInformationTest {
	...
	static void init() throws IOException {
		benchmark = new Tinylog2Benchmark(LocationInfo.FULL);
		benchmark.configure();
	}
	...
	void discard() throws InterruptedException {
		...
		String filename = Configuration.get("writer.file");
	...
	void output() throws InterruptedException {
		...
		assertThat(path)
			...
			.matches("\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2} - main - org\\.tinylog\\.benchmarks\\.logging\\."
				+ "tinylog2\\.Tinylog2Benchmark\\.output\\(\\) - INFO: Hello 42!" + NEW_LINE);
	}
