...
public final class FileWriter implements Writer {
    ...
    /**
     * @param format The output format for rendering log entries
     ...
     * @throws Exception If failed to access the target log file
     */
    ...
    public void log(LogEntry entry) throws Exception {
        try {
            ...
            if (!policy.canAcceptDataRecord(data.length - bom.length)) {
                close();
     ...
     * @throws Exception If failed to create the log file or to initialize the policy
     */
    private static LogFile createLogFile(Path file, byte[] bom, boolean append, Policy policy) throws Exception {
        ...
        if (logFile.isNewFile() && bom.length > 0) {
            policy.canAcceptDataRecord(bom.length);
            logFile.write(bom, 0);
        }
