...
class ByteBuffer {
    ...
    /**
     * Writes all stored bytes to any {@link DataOutput} implementation (e.g. {@link RandomAccessFile}).
     *
     * @param output The destination for writing the stored bytes
     * @return The remaining space in bytes (max size - current size)
     * @throws IOException If failed to write to the passed data output
     */
