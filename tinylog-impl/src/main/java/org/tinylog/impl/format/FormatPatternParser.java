...
public class FormatPatternParser extends AbstractPatternParser {
    ...
    /**
     * @param context The tinylog context to use for placeholders and styles
     */
    public FormatPatternParser(TinylogContext context) {
        this.context = context;
        this.logger = context.getLogger();
        this.placeholderBuilders = PlaceholderBuilder.load(context.getLoader());
        this.styleBuilders = StyleBuilder.load(context.getLoader());
    }
     ...
     * @return A renderable placeholder
     */
    ...
    private Placeholder createPlaceholder(String originalPattern, String resolvedPattern) {
        ...
        if (builder != null) {
            try {
                ...
                logger.log(Level.ERROR, ex, "Failed to create placeholder for \"{}\"", originalPattern);
            }
        }
    ...
    private Placeholder applyStyle(Placeholder placeholder, String style) {
        ...
        if (builder == null) {
            logger.log(Level.ERROR, "Invalid style \"{}\"", style);
        } else {
            try {
                return builder.create(context, placeholder, configuration.getValue());
            } catch (RuntimeException ex) {
                logger.log(Level.ERROR, ex, "Failed to create style for \"{}\"", style);
            }
        }
