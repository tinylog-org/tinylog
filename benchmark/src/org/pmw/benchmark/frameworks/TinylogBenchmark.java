...
public final class TinylogBenchmark implements Benchmark {
	...
	public void write(final long number) {
		Logger.trace("Trace: {0}", number);
		Logger.debug("Debug: {0}", number);
		Logger.info("Info: {0}", number);
		Logger.warn("Warning: {0}", number);
		Logger.error("Error: {0}", number);
	}
	...
	public boolean calculate(final List<Long> primes, final long number) {
		for (Long prime : primes) {
			if (number % prime == 0L) {
				Logger.trace("{0} is not prime", number);
				return false;
			}
		}
		Logger.info("{0} is prime", number);
		return true;
