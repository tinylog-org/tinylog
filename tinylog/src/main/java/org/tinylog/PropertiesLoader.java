...
/**
 * Loads configuration for {@link org.tinylog.Logger Logger} from properties.
 */
final class PropertiesLoader {
	...
	private static Writer readWriter(final Properties properties, final String propertyName, final String writerName) {
		for (Class<?> implementation : findImplementations(Writer.class)) {
			org.tinylog.writers.PropertiesSupport propertiesSupport = implementation.getAnnotation(org.tinylog.writers.PropertiesSupport.class);
	...
	private static Writer loadWriter(final Properties properties, final String propertiesPrefix, final org.tinylog.writers.Property[] definition,
	...
	private static Object[] loadParameters(final Properties properties, final String propertiesPrefix, final org.tinylog.writers.Property[] definition) {
	...
	private static Labeler parseLabeler(final String string) {
		...
		for (Class<?> implementation : findImplementations(Labeler.class)) {
			org.tinylog.labelers.PropertiesSupport propertiesSupport = implementation.getAnnotation(org.tinylog.labelers.PropertiesSupport.class);
			if (propertiesSupport != null) {
	...
	private static Policy parsePolicy(final String string) {
		...
		for (Class<?> implementation : findImplementations(Policy.class)) {
			org.tinylog.policies.PropertiesSupport propertiesSupport = implementation.getAnnotation(org.tinylog.policies.PropertiesSupport.class);
			if (propertiesSupport != null) {
				if (name.equalsIgnoreCase(propertiesSupport.name())) {
